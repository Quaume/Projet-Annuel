...
if(isset($img_profile) && !empty($img_profile['name'])){
	...
	if($img_profile['size'] <= $max_size){
		...
		if(in_array($extensionUpload, $extension)){
			...
			$path = "../ressources/img-profile".$id.".".$extensionUpload;
			//On va deplacer se fichier stocker temporairement et lz placer dans path
			...
			if($result){
				...
				$queryPrepared->execute(array(
					'img_profile'=>$id.'.'.$extensionUpload,
					'id'=>$id));	
			}else{
				$error[] = "Failed import";
			}
		} else{
			$errors[] = "Your image format is not correct! (jpeg, jpg, png)";
		}
	}else{
